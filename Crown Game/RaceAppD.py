...
class RaceApp:
    ...
    def __init__(self):
        ...
        self.textTrialBFont = ctk.CTkFont("Century Gothic", 40, "bold")
        self.textTrialFont = ctk.CTkFont("Century Gothic", 100)
    ...
    def next_instruction(self):
        ...
        if self.instructionNum % 3 == 0:
            ...
            # MOVE DRONE FORWARD
            # self.c.send("MoveForward")
            self.drone.move_forward(60, "cm", 0.5)
            time.sleep(3)
    ...
    def right_callback(self, data):
        ...
        if self.instructionNum % 3 == 1:
            ...
            if DOMI == "RIGHT":
                ...
                prob /= NONDOMITHRESHOLD
            prob *= 100
            prob = min(prob, 100)
            self.rightProbability.set(str(prob))
            if prob >= 100:
                self.next_instruction()
    ...
    def left_callback(self, data):
        ...
        if self.instructionNum % 3 == 0:
            ...
            prob *= 100
            prob = min(prob, 100)
            self.leftProbability.set(str(prob))
            if prob >= 100:
                self.next_instruction()
...
class RacePage(ctk.CTkFrame):
    ...
    def start_race(self):
        if not self.controller.raceStarted:
            ...
            # self.controller.c.send("Takeoff")
